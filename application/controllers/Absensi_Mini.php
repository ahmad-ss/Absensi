...
class Absensi_Mini extends CI_Controller
{
	...
	{
		$data['title']='Absensi Pegawai Staff';
		...
		$data['Tgl']=$tanggal;
        $data['breadcrumb']='';
		$data['divisi'] = $this->crud->getDataQuery("Select * From Sdm_Absensi Where Kd_Bagian='01' And TanggalAbsen='".$tanggal."' Order By No_Urut")->result();
		$data['Karyawan']= $this->crud->getDataQuery("Select * From Sdm_Pegawai Where Kd_Bagian='01' And Aktif='1' Order By No_Urut")->result();
		$data['Status']=$this->crud->getDataQuery("Select * From Sdm_Status Order By id")->result();
		
		$this->template->viewAdmin('View_Pegawai',$data);
	}	
	public function SaveAbsensi_Staff()
	{
		...
		redirect(Absensi_Mini,List_Staff);
	...
	public function List_Harian_umum()
	{
		$data['title']='Absensi Pegawai Harian Umum';
		$check=$this->crud->getDataQuery("Select TanggalAbsen From sdm_absensi Order By TanggalAbsen Desc Limit 1");
		if ($check->num_rows()!=0) 
        {
			foreach ($check->result() as $row)
		...
		$data['Tgl']=$tanggal;
        $data['breadcrumb']='';
		$data['divisi'] = $this->crud->getDataQuery("Select * From Sdm_Absensi Where Kd_Bagian='02' And TanggalAbsen='".$tanggal."' Order By No_Urut")->result();
		$data['Karyawan']= $this->crud->getDataQuery("Select * From Sdm_Pegawai Where Kd_Bagian='02' And Aktif='1' Order By No_Urut")->result();
		$data['Status']=$this->crud->getDataQuery("Select * From Sdm_Status Order By id")->result();
